 ...
 */
export function deriveIdempotencyKey(clientSubmissionKey) {
...
export class InMemoryCircleRequestStore {
    records = new Map();
...
export class CircleCommerceExecutor {
    ...
    constructor(options) {
        if (!options.store)
            throw new CircleStoreRequiredError();
    ...
    async prepare(context) {
        ...
        const reference = { amount: context.action.amount, destinationAddress: context.action.recipient };
    ...
    async submit(prepared) {
        ...
        if (existing?.transactionId)
            return this.resolve(clientSubmissionKey, existing.transactionId, idempotencyKey);
        if (existing)
            throw new CircleAmbiguousSubmitError(clientSubmissionKey, idempotencyKey);
        const placeholder = { clientSubmissionKey, transactionId: null, tokenId: this.tokenId, amount: ref.amount, destinationAddress: ref.destinationAddress, walletId: this.walletId, transactionHash: null };
        const { claimed, record: claimedRecord } = await this.store.claim(clientSubmissionKey, placeholder);
        if (!claimed) {
            if (claimedRecord.transactionId)
                return this.resolve(clientSubmissionKey, claimedRecord.transactionId, idempotencyKey);
            throw new CircleAmbiguousSubmitError(clientSubmissionKey, idempotencyKey);
        }
        ...
        const record = { ...claimedRecord, transactionId: result.id };
     ...
     */
    async resolve(clientSubmissionKey, transactionId, idempotencyKey) {
    ...
    async resolveFromResult(clientSubmissionKey, transactionId, result) {
        ...
        const record = await this.store.get(clientSubmissionKey);
        if (record)
            await this.store.set({ ...record, transactionHash: result.txHash });
