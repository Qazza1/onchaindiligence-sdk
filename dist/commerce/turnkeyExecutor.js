...
export class TurnkeyCommerceExecutor {
    ...
    constructor(options) {
        if (!options.store)
            throw new TurnkeyStoreRequiredError();
    ...
    async submit(prepared) {
        ...
        if (!claimed) {
            if (claimedRecord.sendTransactionStatusId)
                return this.resolve(clientSubmissionKey, claimedRecord.sendTransactionStatusId);
            throw new TurnkeyAmbiguousSubmitError(clientSubmissionKey);
        }
        ...
        const record = { ...claimedRecord, sendTransactionStatusId };
     ...
     */
    async resolve(clientSubmissionKey, sendTransactionStatusId) {
