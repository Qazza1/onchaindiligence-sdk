...
export class CommerceOperation {
   ...
   */
  private async assertPreflightAllowed(): Promise<void> {
      ...
      if (outcome.providerReference) {
        try {
          ...
          return pending(this.operationId, {
            ...
            safeNextAction:
              'call op.execute() again -- the transaction is already known; this will retry attaching the same PayBox provider reference to the existing execution binding, without calling useService() or searching for a different transaction again',
            mayAlreadyHavePaid: true,
            executionRequestId,
          })
        }
   ...
   */
  private async attachProviderReferenceStrict(executionRequestId: string, providerReference: string): Promise<void> {
    let res: Response
    try {
  ...
  private async attachProviderReferenceBestEffort(executionRequestId: string, providerReference: string): Promise<void> {
