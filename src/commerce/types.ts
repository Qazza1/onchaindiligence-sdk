...
export interface Receipt {
  ...
  issued_at: string
  action: CommerceAction & { resource: string | null }
  ...
  settlement: { status: SettlementStatus; detail: string | null }
  checks: ReceiptCheck[]
  links: { agent_evidence_bundle_digest: string | null; preflight_receipt_id: string | null }
  limitations: string[]
}
...
export interface OperationFinalizeResponseBody extends ReceiptEnvelope {
  ocd_lifecycle_evidence: { bundle_digest: string; binding_strength: 'TRANSFER_MATCH_ONLY' | 'EXECUTOR_CORRELATED' | 'PAYMENT_IDENTITY_LINKED' } | null
}
