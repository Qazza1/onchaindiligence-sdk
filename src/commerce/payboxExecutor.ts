...
interface PayBoxPreparedReference {
  ...
  atomicAmount: string
  recipient: string
  /** gateway mode only: the expected on-chain payer, frozen at prepare() time from context.action.sender. */
  expectedPayer?: string | null
}
...
export class PayBoxCommerceExecutor implements CommerceExecutor {
   ...
   */
  private async submitGateway(clientSubmissionKey: string, ref: PayBoxPreparedReference): Promise<ExecutionResult> {
  ...
  private refFromRecord(record: PayBoxRequestRecord): PayBoxPreparedReference {
    return {
      payboxRequestId: record.payboxRequestId,
      resourceUrl: record.resourceUrl,
      network: record.network,
